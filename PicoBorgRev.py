...
Warning, this new I²C address will still be used after resetting the power on the device
    ...
    if oldAddress < 0x0:
        ...
        if len(found) < 1:
            ...
            oldAddress = found[0]
    print('Changing I²C address from {0:02X} to {1:02X} (bus #{2:d})'.format(oldAddress, newAddress, busNumber))
    bus = PicoBorgRev()
    ...
    if foundChip:
        ...
        try:
            ...
            print('Missing PicoBorg Reverse at {0:02X}'.format(newAddress))
    if foundChip:
        print('New I²C address of {0:02X} set successfully'.format(newAddress))
    else:
